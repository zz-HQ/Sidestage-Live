module Account::ProfilesHelper
  ...
  def soundcloud_widget(iframe = nil)
    ...
    if @profile.soundcloud.include?("users")
    ...
    elsif @profile.soundcloud.include?("playlists")
  ...
  def youtube_id
    @profile.youtube[/^(?:https?:\/\/)?(?:www\.)?youtu(?:\.be|be\.com)\/(?:watch\?v=)?([\w-]{10,})/, 1]
  end
  ...
  def load_more?
    ...
    count += @profile.pictures.count
    count += 2 unless @profile.youtube.blank?
    count += 2 unless @profile.soundcloud.blank?
